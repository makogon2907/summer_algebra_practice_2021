...
vector<ll> poly_binpow(vector<ll> poly, const vector<ll>& field_poly, bigint deg, ll p) {
    vector<ll> ans = {1};
    while (deg > 0) {
        if (deg % 2 == 1) {
...
void convert_to_message(const vector<ll>& elements, ll p) {
    vector<ll> number;
    vector<ll> deg = {1};
...
int main() {
    ...
    while (getline(cin, input)) {
        ...
        while (public_key.size() < field_poly.size() - 1) {
            public_key.push_back(0);
        }

        getline(cin, input);
        auto salt = get_poly_from_string(input, p);

        while (salt.size() < field_poly.size() - 1) {
            salt.push_back(0);
        }

        bigint deg = private_key;
        bigint bigp = p;
        deg *= (binpow(bigp, field_poly.size() - 1) - 2);

        auto inverse = poly_binpow(public_key, field_poly, deg, p);

        auto ans = poly_mul(salt, inverse, field_poly, p);
